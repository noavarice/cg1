 ...
 * p1 и p2
 */
static QPair<QPoint, QPoint> getCircleCenter(const QPoint& p1, const QPoint& p2, const uint8_t radius)
{
    int dx, dy;
    if (p1.x() > p2.x()) {
        dx = p1.x() - p2.x();
    ...
    if (!dx) {
        return {QPoint(-catheterLength, middle.y()), QPoint(catheterLength, middle.y())};
    }
    if (!dy) {
        return {QPoint(middle.x(), -catheterLength), QPoint(middle.x(), catheterLength)};
    }
    qreal coeff = -dx / dy;
    dx = qCos(coeff) * catheterLength;
    dy = qSin(coeff) * catheterLength;
    return {middle - QPoint(dx, dy), middle + QPoint(dx, dy)};
}
